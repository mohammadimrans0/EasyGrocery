...
import { UserProfile, WishlistItem, Product, Profile } from "@/constants/types";
...
export const useUserStore = create<UserStore>((set, get) => ({
  userId:
    typeof window !== "undefined"
      ? localStorage.getItem("easygrocery_user_id") || null
      : null,
  authToken:
    typeof window !== "undefined"
      ? localStorage.getItem("easygrocery_auth_token") || null
      : null,
  profile: null,
  wishlist: [],
  productData: {},
    ...
    try {
      ...
      toast.error(
        error.response?.data?.detail ||
          `Logout failed. Status: ${error.response?.status}`
      );
    }
  },
  ...
  fetchWishlist: async () => {
    ...
    try {
      ...
      const uniqueProductIds = [
        ...new Set(wishlistItems.map((item) => item.product)),
      ];
      ...
      set({
        error:
          ...
          "An error occurred while fetching wishlist data.",
        isLoading: false,
      });
    }
  },
